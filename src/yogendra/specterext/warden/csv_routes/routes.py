...
csv_routes=WardenService.blueprints['csv_routes']
...
def exportcsv():
    ...
    if transactions.count() == 0:
        return render_template("warden/empty_txs.html", current_app=current_app,form=form)

    filename = (current_app.settings['SPECTER']['specter_login'] + "_" +
...
def importcsv():
    ...
    if request.method == "GET":
        ...
        if filename:
            ...
            df['user_id'] = current_app.settings['SPECTER']['specter_login']
            ...
            accounts = AccountInfo.query.filter_by(
                user_id=current_app.settings['SPECTER']['specter_login']).order_by(
            ...
            for account in trade_account_list:
                ...
                if not curacc:
                    account = AccountInfo(user_id=current_app.settings['SPECTER']['specter_login'],
