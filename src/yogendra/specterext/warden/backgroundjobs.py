...
def background_specter_health():
        ...
        print("...................specter health called......................")
        health = app.warden_specter.is_healthy()
        if health is False:
            message = Message(category='Specter Server',
                            message_txt=f"<span class='text-danger'>Health Check Returned False at {app.warden_specter.base_url}</span>")
            app.message_handler.add_message(message)
            ...
            seeker = app.warden_specter.seek_specter()
            if seeker is not None:
                ...
                message = Message(category='Specter Server',
                                message_txt=f"<span class='text-success'>Found an alternative server at {app.warden_specter.base_url}.<br>Updated to use this server.</span>")
                app.message_handler.add_message(message)
                return
            else:
                message = Message(category='Specter Server',
...
def background_specter_update():
    ...
    message = Message(category='Specter Server',
                      ...
                      notes=f'Trying to reach Specter at {app.warden_specter.base_url}')
    app.message_handler.add_message(message)

    # Is Specter reachable?
    if app.warden_specter.is_reachable() is True:
        message = Message(category='Specter Server',
                          message_txt="<span class='text-success'>Specter Server Found</span>",
                          notes=f'Reachable at {app.warden_specter.base_url}')
        
        app.message_handler.add_message(message)
    else:
        message = Message(category='Specter Server',
                          message_txt="<span class='text-danger'>Specter Server Unreacheable</span>",
                          notes=f'Tried at {app.warden_specter.base_url}')
        
        app.message_handler.add_message(message)
        return

    # Is Authenticated?
    if app.warden_specter.is_auth() is True:
        message = Message(category='Specter Server',
                          message_txt="<span class='text-success'>Successfully Authenticated</span>",
                          notes=f'Reachable at {app.warden_specter.base_url}')
        
        pp.message_handler.add_message(message)
    else:
        message = Message(category='Specter Server',
    ...
    app.message_handler.add_message(message)
    try:
        metadata = app.warden_specter.home_parser(load=False)
    ...
    try:
        ts_t = time.time()
        txs = app.warden_specter.refresh_txs(load=False)
        te_t = time.time()
        message = Message(category='Specter Server',
    ...
    # Check wallets
    wallets = app.warden_specter.wallet_alias_list(load=True)
    if wallets is None:
        app.warden_specter.specter_reached = False
        ...
        for wallet in wallets:
            ts_a = time.time()
            app.warden_specter.wallet_info(wallet_alias=wallet, load=False)
            rescan = app.warden_specter.rescan_progress(
...
def specter_test(force=False):
    ...
    try:
        specter = app.warden_specter.init_session()
        if type(specter) == str:
