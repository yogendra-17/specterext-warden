...
user_routes=WardenService.blueprints['user_routes']
...
def initial_setup():
    ...
    if page == '3' or page == 'specter_connect':
        ...
        try:
            if current_app.warden_specter.home_parser()['alias_list'] != []:
                flash(f"Succesfuly connected to Specter Server at {current_app.warden_specter.base_url}")
        except Exception:
            pass
...
def account():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            hash = generate_password_hash(form.password.data)
            user = User.query.filter_by(username=current_app.settings['SPECTER']['specter_login']).first()
            user.password = hash
            current_app.db.session.commit()
            flash(f"Account password updated for user {current_app.settings['SPECTER']['specter_login']}", "success")
            return redirect(url_for("warden_routes_endpoint.warden_page"))
