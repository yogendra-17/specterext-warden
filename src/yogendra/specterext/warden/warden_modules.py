...
def get_specter_tx(wallet_alias,
                   ...
                   session=None):
    df = pd.DataFrame()
    wallet_list = current_app.warden_specter.wallet_alias_list()
    if wallet_alias not in wallet_list:
        logging.error(f"Wallet {wallet_alias}: Wallet not in current_app")
        return (df)

    t = current_app.warden_specter.refresh_txs(load=True)
...
def warden_metadata():
    ...
    # meta['wallet_list'] = current_app.warden_specter.wallet_alias_list()
...
def specter_df(delete_files=False, sort_by='trade_date'):
    ...
    df = pd.DataFrame()
    try:
        t = current_app.warden_specter.refresh_txs(load=True)['txlist']
        df = df.append(t)
...
def transactions_fx():
    ...
    if not df_sql.empty:
        df_sql = df_sql[(df_sql.user_id == current_app.settings['SPECTER']['specter_login'])]
...
def generatenav(user=None, force=False, filter=None):
    ...
    if not user:
        user = current_app.settings['SPECTER']['specter_login']
...
def regenerate_nav():
    ...
    try:
        ...
        pass
    try:
        generatenav(current_app.settings['SPECTER']['specter_login'], force=True)
    except Exception:
        return
...
def cost_calculation(ticker, html_table=None):
    ...
    if html_table:
        ...
        html["trade_reference_id"] = "<a href='/svc/warden/routes/edittransaction?reference_id=" +\
