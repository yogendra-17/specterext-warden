...
class MessageHandler():
    ...
    def clean_category(self, category):
        self.messages = [item for item in self.messages if item.category != category]
        self.saver()
    ...
    def to_json(self, category=None, sort_reverse=True):
        ...
        sorted_messages = sorted(self.messages, key=lambda x: x.time, reverse=sort_reverse)
        if category:
            o = [item.__dict__ for item in sorted_messages if item.category == category]
        else:
