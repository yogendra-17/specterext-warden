...
#wdd: no need to import these models
...
warden = WardenService.blueprints["routes"]


@warden.route("/", methods=["GET", "POST"])
def index():
...
#     meta = {
#         'tor': current_app.tor,
#         'specter_reached': current_app.warden_specter.specter_reached,
#         'specter_auth': current_app.warden_specter.specter_auth
#     }
...
#     try:
#         specter_version = str(current_app.warden_specter.home_parser()['version'])
...
# wdd: temporarily removed "/" route from here
...
def warden_page():
    ...
    #WARDD , what we can do is login using specter_auth with specter credentials into warden , now the data we will retrieve will be according to specter credentials
    # and that aree laso the warden credential and whereever current_app.settings['SPECTER']['specter_login']e is required we will use the  current_app.settings['SPECTER']['specter_login'] 
    #because this is our current specter user. we are doing this because our user is not getting stored into database for some reason, like the current_user object return null
    #the reason for that might be current_user is returning the specter user and database is storing the warden user.
    # we can't use specter user because that is hardcoded somewhere and we can't switch the account unless we use specter_auth 
    users=User.query.all()
    #WARDD
    ...
    try:
        ...
        return redirect(url_for("warden_routes_endpoint.newtrade"))
    ...
    sorted_wallet_list = []
    try:
        for wallet in current_app.warden_specter.wallet_alias_list():
    ...
    #wdd: edited this for quick fix
    #To do: check what current_app.dowloading should do
...
def specter_auth():
    ...
    if request.method == 'POST':
        ...
        # Try to authenticate
        try:
            current_app.warden_specter.base_url = url
            current_app.warden_specter.login_url = url + 'auth/login'
            current_app.warden_specter.tx_url = url + 'wallets/wallets_overview/txlist'
            current_app.warden_specter.core_url = url + 'settings/bitcoin_core?'
            current_app.warden_specter.login_payload = {
                'username': request.form.get('username'),
                'password': request.form.get('password')
            }
            session = current_app.warden_specter.init_session()
#WARDD start
            hash = generate_password_hash(request.form.get('password'))
            user = User(username=request.form.get('username'), password=hash)
            user_exist = User.query.filter_by(username=request.form.get('username')).first()
            if user_exist and check_password_hash(user_exist.password, request.form.get('password')):
                login_user(user, remember=True)
            else:    
                current_app.db.session.add(user)
                current_app.db.session.commit()
...
#WARDD end                
            ...
            print("...................error...........................",e)
            return redirect(url_for('warden_routes_endpoint.specter_auth'))
        ...
        current_app.warden_specter = Specter()
        current_app.warden_specter.refresh_txs(load=False)
...
def newtrade():
    form = TradeForm()
    #WARDD
    # cuserid=User.query.filter_by(username=current_app.settings['SPECTER']['specter_login'])
    acclist = AccountInfo.query.filter_by(user_id=current_app.settings['SPECTER']['specter_login'])
    accounts = []
    for item in acclist:
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            ...
            trade = Trades(
                user_id=current_app.settings['SPECTER']['specter_login'],
...
def trade_transactions():
    # transactions=Trades.query.all()
    #WARDD
    # cuserid=User.query.filter_by(username=current_app.settings['SPECTER']['specter_login'])
    transactions = Trades.query.filter_by(user_id=current_app.settings['SPECTER']['specter_login'])
...
def edittransaction():
    ...
    if reference_id:
        trade = Trades.query.filter_by(
            user_id=current_app.settings['SPECTER']['specter_login']).filter_by(
                trade_reference_id=reference_id).first()
        if trade.count() == 0:
            abort(404, "Transaction not found")
        id = trade.id

    trade = Trades.query.filter_by(user_id=current_app.settings['SPECTER']['specter_login']).filter_by(
        id=id).first()

    if trade is None:
        abort(404)

    if trade.user_id != current_app.settings['SPECTER']['specter_login']:
        abort(403)
...
def delalltrades():
    ...
    transactions = Trades.query.filter_by(
        user_id=current_app.settings['SPECTER']['specter_login']).order_by(Trades.trade_date)
    ...
    if request.method == "GET":
        Trades.query.filter_by(user_id=current_app.settings['SPECTER']['specter_login']).delete()
        current_app.db.session.commit()
        regenerate_nav()
...
#WARDD: function to print in console from templates
@warden.context_processor
...
#WARDD: context_filter is replaced by pass_context as context_filter is deprecated in jinja 2 version 3.1.x
@jinja2.pass_context
