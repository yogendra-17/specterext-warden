...
api=WardenService.blueprints['api']
...
#WARDD: Filename and seialize is required as argument
...
# @login_required
def specter_json():
    data = current_app.warden_specter.home_parser()
...
def currentspecter_json():
    ...
    data = current_app.warden_specter.wallet_info
    return simplejson.dumps(data, ignore_nan=True)
...
#WARDD: To check for creation of file debug.log in home directory
# @api.route('/log')
# # @login_required
# def progress_log():
...
def aclst():
    ...
    if request.method == "GET":
        ...
        tradeaccounts = Trades.query.filter_by(
            user_id=current_app.settings['SPECTER']['specter_login']).group_by(Trades.trade_account)

        accounts = AccountInfo.query.filter_by(
            user_id=current_app.settings['SPECTER']['specter_login']).group_by(
...
def portfolio_tickers_json():
    if request.method == "GET":
        ...
        df = df[(df.user_id == current_app.settings['SPECTER']['specter_login'])]
...
def generatenav_json():
    if request.method == "GET":
        ...
        if not force:
            force = False
        nav = generatenav(current_app.settings['SPECTER']['specter_login'], force, filter)
        return nav.to_json()
...
def host_list():
    ...
    if request.method == "GET":
        ...
        if delete is not None:
            ...
            return redirect(url_for("warden_routes_endpoint.running_services"))
...
def drawdown_json():
    ...
    if ticker == "NAV":
        data = generatenav(current_app.settings['SPECTER']['specter_login'])
        data = data[["NAV_fx"]]
